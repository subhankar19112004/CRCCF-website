// src/pages/mainPages/contact/RequestDetails.jsx
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link } from 'react-router-dom';
import { FiChevronLeft, FiSend, FiUploadCloud, FiFileText } from 'react-icons/fi';
import { requestFormSchema } from '../../../data/contactUs/requestData';
...
// --- 1. PURPLE-THEMED ANIMATED INPUT ---
const AnimatedInput = ({ field, value, onChange, onToggle }) => {
    ...
    // THEME CLASSES: Royal Purple
    const activeBorder = "border-purple-700 shadow-md bg-white";
    const inactiveBorder = "border-gray-300";
    const activeLabel = "top-2 text-xs text-purple-800 font-bold";
    ...
    return (
        ...
        >
            ...
            {/* WHATSAPP LOGIC TOGGLE (Purple Accent) */}
            {field.hasCopyToggle && (
                <div className="absolute top-4 right-4 flex items-center z-20">
                    <label className="flex items-center cursor-pointer text-xs font-bold text-gray-500 hover:text-purple-700 transition-colors select-none">
                        <input 
                            type="checkbox" 
                            className="mr-2 accent-purple-700 h-4 w-4 cursor-pointer"
...
const FileUploadInput = ({ field }) => {
    return (
        <div className={`mb-6 ${field.gridSpan === 2 ? 'col-span-1 md:col-span-2' : 'col-span-1'}`}>
            ...
            <div className="border-2 border-dashed border-gray-400 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-purple-50 hover:border-purple-700 transition-all group bg-gray-50">
                <div className="p-4 bg-white rounded-full mb-4 shadow-sm group-hover:bg-purple-100 transition-colors">
                    <FiUploadCloud className="w-8 h-8 text-gray-600 group-hover:text-purple-700" />
                </div>
...
const RequestDetails = () => {
    ...
    useEffect(() => {
        const initialData = {};
        requestFormSchema.forEach(section => {
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    ...
                    <div className="text-center mb-12">
                        <h1 className="text-5xl font-extrabold text-gray-900 mb-4">
                            Submit a <span className="text-purple-800 underline decoration-purple-300 decoration-4 underline-offset-4">Formal Request</span>
                        </h1>
                        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                            Initiate official services, investigations, or partnerships with our dedicated team.
                        </p>
                    </div>
                    ...
                    <motion.div 
                        ...
                        className="bg-white rounded-2xl shadow-2xl overflow-hidden border-t-8 border-purple-900" // Bold top border
                    >
                        {/* Loop through Sections */}
                        {requestFormSchema.map((section, index) => (
                            <div key={index} className="p-8 md:p-10 border-b border-gray-100 last:border-0">
                                ...
                                <h2 className="text-2xl font-extrabold text-gray-900 mb-8 flex items-center">
                                    <span className="bg-purple-900 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm mr-3 shadow-md">
                                        {index + 1}
                                    </span>
                        ...
                        {/* Submit Action Area - PURPLE BUTTON */}
                        <div className="p-10 bg-purple-50 flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex items-center text-gray-600 text-sm">
                                <FiFileText className="w-5 h-5 mr-2 text-purple-800" />
                                <span>Official requests are reviewed within <strong>3-5 Business Days</strong>.</span>
                            </div>
                            <motion.button
                                whileHover={{ scale: 1.02 }}
                                whileTap={{ scale: 0.98 }}
                                className="px-12 py-4 bg-purple-900 text-white text-lg font-bold rounded-xl shadow-xl hover:bg-purple-800 transition-colors flex items-center gap-3 w-full md:w-auto justify-center"
                            >
