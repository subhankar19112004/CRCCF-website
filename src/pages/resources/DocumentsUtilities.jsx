...
import { Download, FileText, ClipboardList, FileSpreadsheet } from "lucide-react";

const DocumentsUtilities = () => {
  const handleDownload = (type) => alert(`Downloading ${type} Documents & Utilities...`);

  const documents = [
    "GDPR Compliance Templates",
    "ISO/IEC 27001 Security Policy Docs",
    "Incident Handling Report Format",
    "Cybersecurity Job Role SOPs",
    "IT Asset Register Templates",
    "Penetration Testing Result Formats",
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-8 lg:px-16">
      ...
      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-gray-600 max-w-3xl mb-8">
        Ready-to-use templates, policies, and utilities for cybersecurity and IT compliance.
      </motion.p>
      ...
      <motion.ul className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {documents.map((item, idx) => (
          <motion.li key={idx} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}
            ...
            <FileText className="text-green-500"/> {item}
          </motion.li>
        ))}
      </motion.ul>
    </div>
...
export default DocumentsUtilities;
